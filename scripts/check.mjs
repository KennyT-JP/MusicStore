 ...
 *   ├─ flutter test                 （2026-08-16 時点 747 件）
 *   ├─ functions の単体テスト       （同 154 件）
 *   ├─ functions の統合テスト       （同 215 件。起動〜後片付けまで自動）
 *   └─ セキュリティルール           （同 202 件。同上・専用ポートで並走）
 *
 * **件数は実行するたびに変わる。** ここの数字は目安で、正しい値は
 * このスクリプトの出力（各行末の「... passed」）を見ること。
 * 書き写した数は必ず古くなる（監査 第5回で 956→1318 のずれを検出）。
 *
