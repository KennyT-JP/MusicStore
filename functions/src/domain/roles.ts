 ...
 * 以外の意味を持ってはいけない。リンクに役割を書き込むと、
 * その URL を渡すこと自体が書き込み権限を配ることになる。
 *
 * 役割を決めるのはここ 1 か所だけで、リンクの中身は見ない。
 * **「メンバーになる」を選んだ人は、必ず Super User で入る**。
 * リンクを配るのは相手を招いた人なので、招かれた側は曲を足せる。
 * 見るだけでよい人のために「メンバーにならずに見る」がある——
 * 参加を選んでも書けないままでは、2 つの選択肢が同じ意味になる。
 *
 * **2026-08-10 に readOnly から戻した。** 2026-08-08 に
 * 「リンクに役割を持たせるな」という指摘を受けたとき、リンクから
 * 役割を外すと同時に、**指摘されていない初期役割まで引き下げて**
 * いた（依頼者の承認なし）。リンクが役割を運ばないことと、
 * 参加した人がどの役割になるかは、別の話である。
 *
 * 役割を上げ下げするのは、この後はリスト管理者の仕事（5.4）。
 * URL が広まったと分かったときは、リンクを取り消す（3.3）。
 */
export const INITIAL_JOIN_ROLE: ListRole = 'superUser';
