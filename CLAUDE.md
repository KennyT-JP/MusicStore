...
| `flutter test` | 438 |
| `functions` の単体テスト | 126 |
| `functions` の統合テスト | 193 |
| セキュリティルール | 202（Firestore 186・Storage 13・見張り 3） |

（件数は 2026-08-15 時点。**正本は `scripts/check.mjs` の出力**）
...
`C:\Codes\共有ドキュメント\` にあります。**このリポジトリには入っていません。**

- `ナレッジベース.md` — 「この場合はこうすべし」。**設計・実装に入る前**に関係する節を読む
- `アンチパターン集.md` — 実際に踏んだ失敗（事象 → 原因 → 回避）。**監査の基準の 1 つ**

**このプロジェクトで得た知見は、一般化して上の 2 つへ追記します**（固有名詞は出典欄へ）。
同じ形の失敗が既にあれば、新しい項目を作らず既存項目に事例を足します。
