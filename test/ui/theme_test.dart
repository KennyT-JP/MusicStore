...
void main() {
  for (final brightness in Brightness.values) {
    group('配色（${brightness.name}）', () {
      ...
      test('文字と背景の明度差が確保されている（WCAG の基準ではない）', () {
        // **これは WCAG（4.5:1）の判定ではない。**
        //
        // 見ているのは「本文の文字 × 背景」**1 組だけ**で、しかも
        // 明るさの**差**（比ではない）。差 0.4 は、明るい背景に
        // 中間色の文字を置いた組み合わせ（比にすると 2:1 程度）を
        // 通してしまう。補助的な説明文・ボタンの中の文字・チップは
        // そもそも見ていない。
        //
        // **配色が黙って戻っていないかを見る回帰テスト**であって、
        // 読みやすさを保証するものではない。基準を上げるかどうかは
        // 2026-08-15 に依頼者と話し、**いまは上げない**と決めた
        // （招待制で使う範囲では実害が小さいため。docs/BACKLOG.md）。
        //
