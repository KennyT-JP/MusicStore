...
**実行するのは 1 つのコマンドです**（約 3 分・並列・別窓なし）。
...
| `flutter test` | 419 |
| `functions` の単体テスト | 123（サーバー側のドメインロジック・通知） |
| `functions` の統合テスト | 96（エミュレータで通し確認） |
| セキュリティルール | 191（Firestore 175・Storage 13・書き方の見張り 3） |
...
> | 症状 | 原因 | 対処 |
> | --- | --- | --- |
> | 追加したアイコンが出ない | 配信のキャッシュ指定でフォントを `immutable` にしていた | 設定は修正済み。既に開いた端末は一度だけ **Clear site data** |
> | 呼び出しが `internal` で失敗 | Cloud Run の呼び出し許可が入っていない | [docs/SETUP.md](docs/SETUP.md)「呼び出し可能関数が `internal` で失敗するとき」 |
> | 再生できない | 足した部品が組み込まれないままビルドされた | `deploy.mjs` が自動で `flutter clean` するよう修正済み |
>
> **3 件とも `flutter test` は緑のままでした。**
> 手元で動くのに配信先で動かないときは、アプリの外側（ビルドの生成物・
> 配信の設定・キャッシュ・クラウド側の権限）を疑ってください。
...
| メール確認待ち | 再送・確認の取り直し |
| パスワード再設定 | リセットリンクの送信 |
| ホーム（参加リスト一覧） | リスト名・項目数・自分の役割。管理者には容量と共有リンクのコピー（3.3） |
...
| サイト管理（4 画面） | 申請承認・リストと容量・**ユーザーの追加／無効化／削除**（11.1）・サイト設定 |
...
| ユーザー管理（11.1） | 追加・無効化・有効化・削除。**無効は戻せる／削除は戻せない**。削除では曲と音源ファイルを消し、コメントは残す |
