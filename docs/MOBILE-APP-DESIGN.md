...
#### 5-11-5b. `main` への push で自動ビルド（2026-08-24 の指示）

**当初は Codemagic の画面から手動で「Start new build」を押す運用だった。**
2026-08-24 に、`main` への push で自動的に始まるよう変更した
（`codemagic.yaml` の `triggering`）。

「push のたびに走らせるか、都度選びたい」という依頼だったが、
**Codemagic に「push ごとに尋ねる」仕組みは無い**（CI は自動起動か
手動起動のどちらか）。代わりに Codemagic 標準の**コミットメッセージに
`[skip ci]` を含めるとそのビルドを飛ばす機能**を使い、「基本は自動、
止めたい push だけ個別に見送る」形にした。

- 対象は `main` のみ。`dev`（検証環境向け）は含めない
- `submit_to_testflight: false` のままなので、**自動で出るのは内部
  テスター（最大 100 人）まで。** 外部テスト・App Store 提出は今も手動
- ビルド分数を毎回消費する点に注意（1-2 の「Codemagic の無料枠は未確認」参照）
