...
> **作業ブランチについて。** この文書を書いた時点のリポジトリは `main` に
> います（`git branch --show-current` = `main`）。作業は `dev` で行う
> 取り決めなので、**着手する人は先に枝を確認してください。**
...
| 要るもの | 費用 | リードタイム | 状況 |
| --- | --- | --- | --- |
| **Apple Developer Program** | 年 $99 | **申込から有効化まで実績 3 日**（公称「最大 48 時間」より長い） | **Session Concierge で登録済みのはず。要確認。** 済んでいれば音源創庫は App ID の追加登録だけ |
| **Google Play Console** | 初回 $25 | 個人は本人確認書類の提出あり。**登録から審査完了まで数日** | 同上。**要確認** |
| **Codemagic のアカウント** | **無料枠の有無は未確認** | — | **音源創庫用に別アプリを登録する必要があります。** build minutes を Session Concierge と共有するのか別枠なのかは**画面で確認が要ります**（どの文書にも記述がありません） |
| **Android の署名鍵（keystore）** | — | — | **音源創庫用に新規作成。** Session Concierge の鍵は流用しません（別アプリなので） |
| **App Store Connect の API キー（`.p8`）** | — | — | **ダウンロードは 1 回きり**。役割は **App Manager 以上**。既存のものを流用できるか要確認 |

> **「Session Concierge で登録済みのはず」は断定していません。**
> Apple Developer Program も Play Console も**アカウント単位**の契約なので、
> 同じ Apple ID / Google アカウントを使うなら**追加登録だけで済むはず**です。
> ただし**確かめていません。** 着手前に、依頼者に次の 3 点を確認してください。
>
> 1. Apple Developer Program は有効か（期限切れになっていないか）
> 2. Play Console のデベロッパーアカウントは有効か
> 3. Codemagic のアカウントで、もう 1 アプリ登録できるか（無料枠の残り）
>
> **1 が切れていると、そこから 3 日は動けません。** 最初に確かめてください。
...
> つまり **iOS だけが先に世に出て、Android が遅れる**という順序になります。
> **論点 1 の「iOS / Android 同時」は「開発を同時に進める」の意味であって、
> 「同じ日に公開できる」ではありません。** ここを依頼者と揃えてください。
...
| 項目 | 実績・見込み |
| --- | --- |
| Apple Developer Program の有効化 | **実績 3 日**（公称「最大 48 時間」より長い）。**済んでいれば 0** |
| Play Console のデベロッパー登録 | 数日（本人確認あり）。**済んでいれば 0** |
...
| 8 | **`Uri.base` の始末**（ネイティブ用の固定ドメイン） | 手元 | **これを飛ばすと、共有 URL を作った瞬間に例外で落ちます** |
| 9 | **`AndroidManifest` と `build.gradle.kts` を直す**（バックアップ禁止・デバッグ署名フォールバック廃止） | 手元 | 10 の前 |
| 10 | **署名を通す**（keystore・Codemagic の登録） | 依頼者 | 9 のあと |
...
| iOS の設定 | **`ios/Runner/Runner.entitlements`**（新規）に `com.apple.developer.applesignin` = `[Default]`。**この記述が無いと、実装しても呼んだ瞬間に失敗します** |
...
### 5-8. `Uri.base` の始末（**飛ばすと共有 URL で落ちます**）
...
| 直すもの | 場所 |
| --- | --- |
| **ネイティブ用の固定ドメイン定数を 1 箇所に集約する** | 新規（`lib/ui/share_url.dart` か `lib/env/app_environment.dart`） |
| `buildShareUrl` の `base ?? Uri.base` | `lib/ui/share_url.dart:18` |
| 起動時のディープリンク取得 | `lib/main.dart:42`。**ネイティブでは常に空になります。** 「モバイルでは起動 URL を扱わない」と決めるなら、**そう明示してください** |

> **本番と検証で URL が違います**（本番は独自ドメイン、検証は `*.web.app`）。
> **`--dart-define` で切り替わる `lib/env/app_environment.dart` に置くのが
> 自然です。** ここを間違えると、**検証環境のアプリが本番の URL を配ります。**

> **共有リンクをアプリで直接開かせたい場合、前例がありません。**
> Session Concierge の `<intent-filter>` は **LAUNCHER の 1 つだけ**で、
> ディープリンク（`VIEW` / `BROWSABLE` / `autoVerify`）の宣言はありません。
> **QR・共有 URL はブラウザで開かせる設計です。**
> 音源創庫も同じでよいか、**依頼者に確認してください**（9 節）。
...
> 機種変更で移行されません（再ダウンロードになります）。**
> **これを許容できるか、依頼者に確認してください**
...
| --- | --- |
| **著作権の所在** | いま**言及がゼロ** |
| **アップロードしてよいもの／いけないもの** | 権利を持たない音源を上げられると、運営が責任を負います |
| **ダウンロードした音源の扱い** | **端末外へ持ち出さない**こと |
...
> **文章そのものは依頼者の判断です。** 上は項目の一覧であって条文案では
> ありません。運営者の責任範囲に関わるので、こちらで決め打ちしません。
...
> **EU のトレーダーステータスに注意してください。**
> 届け出ると **名称・住所・電話・メールが EU の App Store 製品ページに
> 公開されます。** **先に EU を外して出し、体制が整ってから足す**という
> 順序が取れます。ただし App Store Connect の地域区分「ヨーロッパ（42）」は
> **EU 27 か国＋非 EU 15 か国**で、まとめて外すとイギリス・スイス・
> ノルウェー等も落ちます。**依頼者の判断が要ります。**
...
| 15 | **`buildShareUrl` が `Uri.base` に直接依存していない** | `lib/ui/share_url.dart` を読む | **モバイルで共有 URL を作った瞬間に例外**（2-5） |
...
| **ディープリンク（`VIEW` / `BROWSABLE` / `autoVerify`）** | Session Concierge も持っていません（QR・共有 URL はブラウザで開かせる設計）。**音源創庫でやりたいなら前例が無く、別途設計が要ります**（9 節） |
...
### 8-2. Play アプリ署名鍵の SHA-1 を登録し忘れる

> **AAB を Play へ上げると、Google が署名し直します。**
> 端末に届くアプリの署名は手元の鍵ではなく **Play の鍵**になるため、
> Firebase に手元の鍵の SHA-1 しか登録していないと、
> **ストアから入れた人だけ Google ログインが失敗します。**
...
| どの SHA-1 を見るか | 「ポスト量子暗号鍵」ではなく**従来の鍵**の SHA-1 |
| **両方登録する** | 手元のアップロード鍵の SHA-1 **と** Play アプリ署名鍵の SHA-1 |
...
| **ディープリンク** | Session Concierge の `<intent-filter>` は LAUNCHER の 1 つだけ。**共有リンクをアプリで直接開かせたいなら前例がありません** |
| **フォントの同梱量** | Session Concierge は 1.35MB × 2、音源創庫は 2.36MB × 1。**同じ問題ですが数字が違います** |
| **Codemagic の料金・無料枠** | **どの文書にも記述がありません。画面で確認が要ります** |

> **唯一移植できる考え方**（ダウンロード機能へ）:
> `data_extraction_rules.xml` で全ドメインを除外する以上、
> **端末に置いたものは端末間移行で持ち出されません。**
> **ダウンロード済み音源は機種変更で移行されない（再ダウンロードになる）**
> ことを、依頼者が許容できるか確認が要ります（5-9-1）。
