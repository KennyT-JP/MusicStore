...
| 18 | **サイト管理者もプレミアムが要るか** | **要る。例外を作らない。** 判定は「**メンバーかつプレミアム**」の 1 つだけ |
...
**運営者に実害はありません。** サイト管理者は
`extendPremium`（`functions/src/callable/premium.ts:26`）で自分の期限を延ばせますし、
ダウンロードしたいリストにはメンバーとして入れます。
...
| B | サイト管理者もプレミアムが要るか | **要る。例外を作らない**（論点 18） | 2.2・5.1・5.2 |
