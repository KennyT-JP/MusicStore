...
| 18 | **サイト管理者もプレミアムが要るか／メンバーでなければ不可か** | **どちらも不要（2026-08-22 上書き）。** サイト管理者は下位の権限をすべて包含する（仕様書 4.1／4.2）ので、**プレミアム軸・メンバー軸の両方で実効的に扱う**→サイト管理者は**全リストでダウンロード可**（メンバー登録が無いリストも）。旧決定「プレミアムが要る・メンバーでなければ不可・例外を作らない」は下の 2.2 に履歴として残す |
...
#### 論点 18 の効き方（設計が 2 段階で変わっています）

**この論点は、初稿・中間・現在の 3 つの状態を経ています。** 混乱を
避けるため経緯を残します。**現在の設計は表のいちばん右の列です。**

| | 初稿 | 中間（premium 軸だけ上書き） | **現在（2026-08-22 に確定）** |
| --- | --- | --- | --- |
| premium 軸 | サイト管理者も**プレミアムが要る** | 実効プレミアム（有効 or サイト管理者） | **実効プレミアム**（同左） |
| メンバー軸（サーバー） | `members` が有るだけ | `members` が有るだけ | **`isMember \|\| isSiteAdmin`** |
| メンバー軸（クライアント） | `access.role != null` | `access.role != null` | **`access.role != null \|\| access.isSiteAdmin`** |
| サイト管理者がメンバーでないリスト | できない | できない | **できる（全リスト）** |
| サイト管理者がメンバーであるリスト | できる | できる | **できる** |

**なぜ最後にメンバー軸も揃えたか。** 依頼者が「サイト管理者 ＞
プレミアムユーザー ＞ 登録ユーザー ＞ 未登録ユーザーであり、**上位の
ユーザーは下位の全ての権限を包含する**」と明示したためです（仕様書 4.1／
4.2「サイト管理者は全リストの項目を扱える」）。premium 軸だけ包含して
メンバー軸を据え置くと、「サイト管理者なのに参加していないリストは
落とせない」という、包含の原則と食い違う穴が残ります。

**判定は 2 つに戻りました（メンバーか／サイト管理者か）。** 初稿はこれを
避けて例外を廃していました（危険 8）が、依頼者の権限モデルを優先します。
その代わり、**クライアントとサーバーで同じ式**にします——サーバーは
`verifyDownloadAccess` で `lists[id] = isMember || isSiteAdmin ? 'member' : …`、
クライアントは `Permissions.canDownload` で `role != null || isSiteAdmin`。
サーバーがサイト管理者に `member` を返すので、**落とした後の再起動でも
消えません。** premium 軸も同じく `premiumActive: isPremiumActive(...) ||
isSiteAdmin`、クライアントは `canDownloadProvider` が実効プレミアム
（`isPremiumOrAdminProvider`）を渡します。旧決定（プレミアムが要る・
メンバーでなければ不可）は履歴として残します。
...
export const verifyDownloadAccess = onCall({ region: REGION }, async (request) => {
  ...
  const isSiteAdmin = isSiteAdminRequest(request);    // callable/access.ts
  ...
  if (listIds.length > 0) {
    ...
    listIds.forEach((id, i) => {
      // **サイト管理者は members が無くても member**（仕様書 4.2「全リストの
      // 項目を扱える」／論点 18 の上書き）。クライアント側（5.2）も
      // `role != null || isSiteAdmin` と同じ式にして、食い違いで端末の
      // ファイルが消えないようにする。
      lists[id] = snaps[i].exists || isSiteAdmin ? 'member' : 'notMember';
    });
  }
...
| サイト管理者の判定 | `isSiteAdminRequest`（`functions/src/callable/access.ts`）で読み、premium 軸とメンバー軸の両方へ渡す（仕様書 4.1／4.2。旧・論点 18 の「import しない」を上書き） |
...
/// **canView は使わない**——`canView` は `effectiveRole != null || isViewer`
/// なので閲覧者が通ってしまう（論点 9）。
///
/// **サーバー側（verifyDownloadAccess）も、サイト管理者なら members が
/// 無くても member を返す**（`isMember || isSiteAdmin`）ので、
/// クライアントとサーバーで揃う。
///
/// **isPremium には実効プレミアム（プレミアム有効 or サイト管理者）を渡す。**
...
**`role != null || isSiteAdmin` は「メンバー登録があるか、サイト管理者」
と同じです。** サイト管理者は全リストを扱えるので、参加していない
リストでも落とせます（仕様書 4.2）。
...
| 書き方 | 何が起きるか |
| --- | --- |
| `access.canView` | **閲覧者が通ります。** `canView` は `effectiveRole != null \|\| isViewer` です（`role.dart:99`）。閲覧者は落とせません（論点 9） |
| `\|\| isSiteAdmin` を落とす | **サイト管理者が参加していないリストで落とせなくなります。** サーバーは `member` を返すのに、クライアントがボタンを出さない食い違いになります（仕様書 4.2） |

**クライアントとサーバーで同じ式にすること。** サーバーは
`isMember || isSiteAdmin` で `member` を返すので、クライアントも
`role != null || isSiteAdmin` にすれば、落とした後の再起動でも消えません。
...
| **サイト管理者で、そのリストのメンバーではない人** | ボタンを**出します**（全リスト／仕様書 4.2。旧・論点 18 を上書き）。サイト管理者は全リストを扱えるので、参加していないリストでも落とせます。サーバーも `member` を返すので、落とした後の再起動でも消えません |
...
| Super User | true | できる |
| リスト管理者 | true | できる |
| **サイト管理者でメンバーではない**（`ListAccess.siteAdmin()`） | true | **できる（全リスト）**（仕様書 4.2。旧・論点 18 を上書き） |
| **サイト管理者でメンバーでもある**（`isSiteAdmin: true, role: readOnly`） | true | **できる** |
| **閲覧者**（`isViewer: true, role: null`） | true | **できない**（論点 9） |
| メンバーでもなんでもない（`ListAccess.none()`） | true | できない |
| Read Only のメンバー | **false** | **できない**（論点 12） |
| 閲覧者 | false | できない |
| サイト管理者でメンバーでもある | **false** | **できない**（`isPremium` を false で渡したとき。実効値の合成は上流の `isPremiumOrAdminProvider` に置き、`canDownload` 自身はプレミアムを導き出さない／5.2） |

**サイト管理者の行は、行きすぎた修正の見張りです。**

| 行 | 何を防ぐか |
| --- | --- |
| **サイト管理者でメンバーではない → できる（全リスト）** | **`\|\| access.isSiteAdmin` を消さないこと。** 消すと、参加していないリストでサイト管理者が落とせなくなり、仕様書 4.2 と食い違います |
| **サイト管理者メンバーでも、渡した `isPremium` が false なら false** | **`canDownload` の中で `\|\| isSiteAdmin` からプレミアムを導き出さないこと。** プレミアムの合成を 1 か所（`isPremiumOrAdminProvider`）に保つための見張りです |
...
- **サイト管理者は `members` が無くても `member` が返ること**（全リスト／
  仕様書 4.2。旧・論点 18 を上書き）。**`siteAdmin` クレームを持つ利用者で
  確かめること。** 同時に、**一般利用者は `members` が無ければ `notMember`**
  であること——`isSiteAdmin` を混ぜる相手を取り違えていないかを守ります
...
### 8. サイト管理者の例外を、片側だけ直してしまう

**この危険は、設計の変遷で性質が 2 回変わりました。** いま守るべき形を
先に書きます——**サイト管理者の例外は、クライアントとサーバーの両方に
あり、同じ式でなければなりません。**

サイト管理者は `lists/{listId}/members/{uid}` を持たない
（`functions/src/domain/roles.ts:46-49`）ので、判定にはリストごとに
`isMember || isSiteAdmin` という例外が入ります。これは仕様書 4.1／4.2
（上位は下位の権限を包含・全リストを扱える）に沿った、**意図した例外**です。

| | 初稿 | 中間（論点 18 で例外を廃止） | **現在（例外を戻した）** |
| --- | --- | --- | --- |
| サーバー | `isSiteAdmin \|\| members` | `members` だけ | **`isMember \|\| isSiteAdmin`** |
| クライアント | `hasAtLeast(readOnly)` | `role != null` | **`role != null \|\| isSiteAdmin`** |
| 判定の数 | 2 つ | 1 つ | **2 つ（意図して）** |

**危ないのは、この例外を片側だけ直すことです。** サーバーに
`|| isSiteAdmin` を足してクライアント（5.2）を忘れると、サイト管理者は
落とせたのに**ボタンが出ず**、逆にクライアントだけ足すと**落とせたのに
次の起動でサーバーが `notMember` を返して消えます。** 初稿はこの食い違いを
避けるために例外そのものを廃していました。いまは依頼者の権限モデルを
優先して例外を戻したので、**両側を必ず同じ式に保つ**ことで食い違いを防ぎます。

テストは両側を固定しています——サーバーは
「サイト管理者は `members` が無くても `member`」（8.2）、クライアントは
「サイト管理者はメンバーでなくてもできる（全リスト）」（`permissions_test.dart`）。
**どちらか片方だけ変えると、対のテストが落ちます。**
...
| B | サイト管理者もプレミアムが要るか／メンバーでなければ不可か | **どちらも不要へ上書き（2026-08-22）。** サイト管理者は下位の権限をすべて包含（仕様書 4.1／4.2）。premium 軸・メンバー軸の両方で実効的に扱い、全リストで落とせる | 2.2・5.1・5.2 |
...
#### B：いったん外した例外を、権限モデルに合わせて戻した

**この論点は 2 回向きが変わりました。**

初稿は「サイト管理者は `members` を持たないから、例外を入れないと端末から
全部消える」と考え、サーバー側に `isSiteAdmin ||` を入れていました
（そして、それがクライアントと食い違う危険を 10 節の危険 8 として
自分で挙げていました）。中間では、その食い違いを避けるために
**例外そのものを廃し**、判定を 1 つに縮めました。

**最終的には、依頼者の権限モデルに合わせて例外を戻しました。** 依頼者が
「上位のユーザーは下位の全ての権限を包含する」と明示したため（仕様書 4.1／
4.2）、サイト管理者が参加していないリストを落とせないのは原則との食い違い
になります。判定は 2 つに戻りましたが、**クライアントとサーバーを同じ式に
保ち、対のテストで固定する**ことで、危険 8（片側だけ直すと消える）を
抑えています。

| | 初稿 | 中間 | 現在 |
| --- | --- | --- | --- |
| 判定の数 | 2 つ | 1 つ | **2 つ（意図して）** |
| 10 節の危険 8 | 「例外を落とすと消える」 | 「例外を戻すと消える」 | **「例外を片側だけ直すと消える」** |

**設計の縮みより、権限モデルの一貫性を優先した**という判断です。
運営者は、参加していないリストでも追加操作なしに全リストを扱えます
（仕様書 4.2）。
