...
**対象は `onCall` の 31 件だけです。** トリガー（`onFileUploaded` など）と
...
**原本を直して、生成し直します。手で `web/help/` を触らないでください。**
...
| | 場所 |
| --- | --- |
| 原本（マニュアル） | `docs/manual/ja.html` / `docs/manual/en.html`（1 枚もの） |
| 原本（プライバシーポリシー） | `docs/manual/legal-ja.html` / `legal-en.html`（本文だけ） |
| 生成物（配信されるもの） | `web/help/{ja,en}/` の目次 1 枚・章ページ 6 枚・`privacy.html` |

**なぜ分けるか。** 広告は読み物のページにだけ置く方針で（仕様 12.5）、
**1 ページが薄いと審査で「有用性の低いコンテンツ」と判定される**ためです。
章を 1 つずつページにすると全部が薄くなるので、6 ページに束ねています。
束ね方は `scripts/build-manual.mjs` の `PAGES`。

- **章を足したら `PAGES` にも足す。** 入れ忘れると生成が止まります
  （配信されない章ができないように、わざと止めています）
- **本文が 1,600 字を下回るページを作れません。** 下回ると生成が止まります
- 画面のヘルプの飛び先は `lib/domain/help_links.dart`。
  **束ね方を変えたらここも直す**（`test/domain/help_links_test.dart` が
  生成物そのものを読んで突き合わせます）
- 生成物はコミットします。`flutter build web` が `web/` をそのまま
  `build/web` へ写すためです
...
| 済 | 内容 |
| --- | --- |
| ✓ | 広告のコードをアプリの画面から外し、読み物のページへ置いた |
| ✓ | `web/ads.txt`（所有者の証明。パブリッシャー ID は公開情報） |
...
> **第 4 回監査（2026-08-10）の最大の反省がこれです。**
> 6 観点を独立に走らせ、対照実験までして「異常なし」とした当日に、
> 依頼者が 3 件の不具合を報告しました。**3 件とも、画面を開けば
> 10 秒で分かるもの**でした。監査はコードと文書の突き合わせで進めたため、
> **画面だけが映っていなかった**のです。
>
> 以降は、配信のたびに**実際に描いて確かめて**ください。

#### 準備

1. Chrome に Claude の拡張を入れ、**サイドパネルを開いてサインイン**する
   （このアプリと同じアカウント）。**入れただけでは繋がりません。**
2. **アプリへのログインは、本人が行う。** パスワードは代理で扱いません。

#### 狭い画面の再現（ここが要点）

**ウィンドウが最大化されていると、幅を変える指示が効きません。**
成功と返るのに `window.innerWidth` は変わらないため、
**広い画面のまま「スマホで確認した」と勘違いします。**

このアプリは `web/index.html` で描画先を `#flutter-host` に指定して
いるので、**その要素の幅を絞れば Flutter はその幅で描き直します。**
ウィンドウの状態に左右されません。

```js
const h = document.querySelector('#flutter-host');
h.style.width = '390px'; h.style.height = '800px'; h.style.overflow = 'hidden';
```

**再読み込みのたびに消えるので、読み込み後に入れ直します。**

#### 画面の読み取り

**中身は DOM から読めません。** CanvasKit がキャンバスに描くため、
アクセシビリティツリーは空です。**スクリーンショットで見てください。**
（読み上げ用の情報が要るときは `flt-semantics-placeholder` を押すと
出ますが、通常の確認では要りません。）

#### 文言が本当に配信されているか

画面を辿らなくても、**配信物そのものを読めば確かめられます。**
共有リンクのように、確かめるためにデータを作ってしまう画面で有効です。

```js
const js = await fetch('/main.dart.js').then(r => r.text());
const esc = s => s.split('').map(c => {
  const n = c.charCodeAt(0);
  return n > 127 ? '\\u' + n.toString(16).padStart(4, '0') : c;
}).join('');
const has = s => js.includes(s) || js.includes(esc(s));
has('メンバーになります。曲やコメントを追加できます。');   // 新しい文言
has('役割によっては曲やコメントを追加できます');            // 古い文言（false になるはず）
```

> **日本語は `\uXXXX` に変換されて入っています。** そのまま検索すると
> **英語だけ見つかり、日本語は全部「無い」と出ます。**
> 上の `esc` を通して比べてください。
> **新しい文言が入っていることと、古い文言が消えていることの両方**を見ます。
> 片方だけだと、二重に入っている状態を見逃します。

#### 毎回見るもの

- [ ] 幅 390px で、**名前や長い文字列が縦一列に潰れていない**
- [ ] ログイン直後・**再読み込み直後**に、権限で出し分ける項目
      （サイト管理の入口・通知の件数）が**最初から出ている**
- [ ] 未ログインで保護された URL を開くと、**戻り先を保った**ログイン画面へ行く
- [ ] コンソールにエラーが出ていない
- [ ] 直した文言が、配信物に入っている（古い文言が消えている）

> **あとから届く値は、届く前の見え方まで見てください。**
> 2026-08-11 の確認では、ホームのリストが 3 秒ほど「0件」と出てから
> 実際の件数に変わりました。**まだ届いていないのに言い切っている**
> 状態で、「空だ」と「取れていない」の区別がつきません（BACKLOG）。
